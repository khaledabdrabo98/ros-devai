    ...
    def action(self, outcome):
        ...
        if self.nombre_bonnes_predis == self.nombre_predis_ennui:
            ...
            if self.memoire.get(self._action) is not None:
                if self.valence_table[self._action][self.memoire[self._action]] == -1:
                    action_suivante = (self._action+1) % 3
                    action_suivante2 = (self._action+2) % 3
                    if self.memoire.get(action_suivante) is not None and self.valence_table[action_suivante][self.memoire[action_suivante]] == 1 :
                        self._action = action_suivante 
                    elif self.memoire.get(action_suivante2) is not None and self.valence_table[action_suivante2][self.memoire[action_suivante2]] == 1 :
                        self._action = action_suivante2
                    else :
                        self._action = action_suivante 
